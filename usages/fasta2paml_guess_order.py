        ...
        if os.path.isdir(species_folder):
            ...
            for infile in os.listdir(species_folder):
                if infile.split('.')[-1] == 'fas-gb':
                    logging.info("returning info for file {}".format(infile))
        ...
        if os.path.isdir(species_folder) and species_folder.name == folder_name:
            for infile in os.listdir(species_folder):
                if infile.split('.')[-1] == 'phylip':
                    logging.info("returning info for phylip file {}".format(infile))
                    return infile


def fasta2phylip(personal_folder, infile, order_list, folder_in, folder_out):
    """ converting fasta to philip-sequential format"""
    logging.info("start converting fasta2phylip")
...
def run_codeml(folder_in, species_folder, item_folder, infile, phylogeny_tree_path, exec_path):
    ...
    logging.info("Return code={} for file number {}, stderr: {}".format(return_code, file_number,
                                                                                          stderr))
...
def wright_order_file(folder_in, species_folder, order_list):
    order_string = ','.join(order_list)
    for infile in os.scandir(folder_in):
        if infile.name == '{}.{}'.format(species_folder, 'order'):
            order_file_path = os.path.join(folder_in, infile.name)
...
def main(folder_in, folder_order, folder_trees, exec_path, folder_out, species, group):
    for species_folder, infile, order_list in get_infile_and_order(folder_in, folder_order):
        ...
        while not guess:
            ...
            fasta2phylip(species_folder, infile, order_list, folder_in, folder_out)
            ...
            logging.info("remove {}".format(seq_philip_file))
            ...
            if not run_codeml(folder_out, species_folder, item_folder, infile, tree_path, exec_path):
                ...
                guess = True
                logging.info('GUESS!Right order for species folder {}={}'.format(species_folder, order_list))
                wright_order_file(folder_order, species_folder, order_list)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    try:
        main(args.i, args.order, args.tree, executable_path, out_dir, int(args.species), int(args.group))
